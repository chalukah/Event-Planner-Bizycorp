...
export function CellEditor({ row, col, cell, store, onClose, onOpenFormatting }: CellEditorProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-border p-2 flex gap-2">
      ...
      />
      {onOpenFormatting && (
        <button onClick={onOpenFormatting} className="inline-flex items-center justify-center rounded border border-border px-2 py-1 text-sm hover:bg-muted" type="button">
